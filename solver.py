...
SOLVED_STATE = "YYYYRRRRGGGGOOOOBBBBWWWW"
...
class NeuralHeuristic:
    def __init__(self, checkpoint_path: str, config_path: str, device: str = "cuda"):
        ...
        self.max_batch_size = 512  # Max batch size for batched inference
        model_cfg = dict(
...
def astar_solve(
    ...
    # Parent tracking: node_id -> (parent_id, move)
    parent_map = {}
    ...
    open_set = [start_node]
    visited = {start_state: 0}  # state -> best g_score
    state_to_node_id = {start_state: start_node_id}  # state -> best node_id
    ...
    try:
        while open_set and metrics["nodes_expanded"] < max_nodes:
            ...
            for current in nodes_to_expand:
                ...
                # Reuse cube object and rotate in place
                temp_cube = magiccube.Cube(2, current.state_str)
                for move in MOVES:
                    temp_cube.rotate(move)
                    new_state = get_cube_state_str(temp_cube)
                    temp_cube.rotate(INVERSE_MOVES[move])  # Undo for next iteration
                    
                    metrics["nodes_generated"] += 1
                    
                    # Check if solved (fast path - avoid creating node if solved)
                    ...
                    # Skip if we've seen this state with better cost (before creating cube)
            ...
            if all_new_states:
                # Convert states back to cubes for heuristic (one-time cost)
                all_new_cubes = [magiccube.Cube(2, state) for state in all_new_states]
                heuristics = heuristic_fn.batch_call(all_new_cubes)
                metrics["heuristic_calls"] += len(all_new_cubes)
                
                for new_state, (parent_id, move, new_g), h in zip(
                    all_new_states, all_new_info, heuristics
                ):
                    new_node_id = get_next_node_id()
                    parent_map[new_node_id] = (parent_id, move)
                    state_to_node_id[new_state] = new_node_id
...
def main():
    ...
    # Load neural heuristic
    neural_heuristic = NeuralHeuristic(args.checkpoint, args.config)
    
    # Run evaluation with neural heuristic
    neural_results = run_evaluation(
        neural_heuristic,
