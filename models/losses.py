...
class MinLossACTLossHead(nn.Module):
    """ACT Loss Head for multi-solution labels (B, num_solutions, seq_len).

    For each example, computes loss against every valid solution and
    backpropagates only through the solution with the minimum loss.
    Exact-match accuracy counts a prediction as correct if it matches
    ANY valid solution.
    """
